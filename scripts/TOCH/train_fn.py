...
bce_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([6]).to(device))
l1_loss = nn.L1Loss()
mse_loss = nn.MSELoss()
...
def model_loss(ho_autoencoder, data):
    ...
    dist_weight = masked_softmax(
        -torch.abs(10 * corr_dist_gt), corr_mask_gt, dim=2
    ) * corr_mask_gt.sum(dim=2, keepdim=True)
    corr_dist_loss = weighted_mse_loss(
        corr_dist_pred[corr_mask_bool],
        10 * corr_dist_gt[corr_mask_bool],
        dist_weight[corr_mask_bool],
    )
...
def train_model(ho_autoencoder, train_loader, vald_loader, opt, device, args):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for term in train_loss_dict:
            print(
                "\t{} {:.5f}".format(term, train_loss_dict[term] / len(train_loader)),
                flush=True,
            )
        ...
        for term in vald_loss_dict:
            print(
                "\t{} {:.5f}".format(term, vald_loss_dict[term] / len(vald_loader)),
                flush=True,
            )
        ...
        if save_int or epoch == num_epochs:
            checkpoint = {
                ...
                "model": ho_autoencoder.state_dict(),
                "opt": opt.state_dict(),
            }
