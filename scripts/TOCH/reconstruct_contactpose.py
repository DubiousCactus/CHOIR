...
# from data.affine_mano import AffineMANO
from dataloading import ContactPose_Single_Frame
...
def joint_acc_loss(x, J_regressor):
    ...
    joints = torch.matmul(
        J_regressor, x.permute(2, 3, 0, 1).contiguous()[:-1].view(778, -1)
    )
    joints = joints.view(16, 3, args.num_init, -1).permute(2, 3, 0, 1).contiguous()
    joints_acc = torch.sum(
        (
            joints[:, 2:]
            - 2 * joints[:, 1 : num_frames - 1]
            + joints[:, : num_frames - 2]
        )
        ** 2,
        dim=-1,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mano_path", type=str, required=True)
    ...
    # load model
    if args.ckpt_path is None or not osp.isfile(args.ckpt_path):
        raise FileNotFoundError("Checkpoint not found")
    ckpt = torch.load(args.ckpt_path, map_location=torch.device("cpu"))

    ho_autoencoder = TemporalPointAE(
        input_dim=11, latent_dim=args.latent_size, window_size=1
    )
    ...
    for f in os.listdir(args.data_path):
        if f.endswith(".pkl") and f.startswith("dataset"):
            dataset_path = osp.join(args.data_path, f)
    ...
    id2objmesh = {}
    with open(osp.join(args.data_path, "objname2id.pkl"), "rb") as f:
        ...
        for objname, objid in objname2id.items():
            ...
            id2objmesh[objid] = obj_path

    seq = ContactPose_Single_Frame(args.grasp_path)
    num_frames = len(seq)
    assert num_frames == 1, "ContactPose is for single frame optimization only"
    ...
    for i in range(
        num_frames
    ):  # Should iterate only once, since it's not a sequence but a single frame
        assert (
            i <= 0
        ), "ContactPose is for single frame optimization only. This loop should never iterate more than once."
        (
            input_rhand_pc,
            obj_pc,
            obj_corr_mask,
    ...
    # window_mid = args.window_size // 2 # Our window is 1
    # batched_input = []
    ...
    # torch.stack(
    # input_features[num_frames - args.window_size : num_frames], dim=0
    # )
    # )
    # else:
    # batched_input.append(
    # torch.stack(input_features[i : i + args.window_size], dim=0)
    # )
    ...
    with torch.no_grad():
        corr_mask_output, corr_pts_output, corr_dist_output = ho_autoencoder(
            batched_input
        )
    ...
    object_contact_pts = (
        torch.cat(object_contact_pts, dim=0).unsqueeze(0).repeat(args.num_init, 1, 1)
    )
    ...
    # affine_mano = AffineMANO(for_contactpose=True)
    ...
    with torch.no_grad():
        ...
        pred_contact_pts = []
        for j in tqdm(range(args.num_init), desc="Selecting best initialization"):
            for k in range(num_frames):
                vert_ids = data_collection[k][0]
                bary_coords = data_collection[k][1]
                pred_contact_pts.append(
    ...
    for i in range(num_frames):
        ...
        hand_mesh_input.write_ply(
            os.path.join(args.out_path, "input_hand_{}.ply".format(i))
        )
