...
class MLPUNetBackbone(torch.nn.Module):
    def __init__(self, time_encoder: torch.nn.Module, bps_dim: int, temporal_dim: int):
        ...
        self.in_ = torch.nn.Linear(bps_dim * self.choir_dim, bps_dim * self.choir_dim)
        self.identity_1 = ResBlock(
            bps_dim * self.choir_dim, bps_dim * self.choir_dim, temporal_dim
        )
        self.down_1 = ResBlock(bps_dim * self.choir_dim, 512, temporal_dim)
        ...
        self.up_3 = ResBlock(512 + 512, 512, temporal_dim)
        self.identity_2 = ResBlock(512, 512, temporal_dim)
        self.out = torch.nn.Linear(512, bps_dim * self.choir_dim, temporal_dim)
