...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        x = (x - x.mean(dim=self.dim, keepdim=True)) / torch.sqrt(
            x.var(dim=self.dim, keepdim=True) + self.eps
        )
        return x
...
class STPointNetBlock(nn.Module):
    def __init__(
        self, input_dim, layer_dims, layernorm=True, global_feat=False, transposed=False
    ):
        super().__init__()
    ...
    def forward(self, x):
        if self.transposed:
            ...
            x = x.permute(0, 3, 1, 2).view(
                batch_size, self.input_dim, window_size * num_points
            )
        ...
        x = x.view(batch_size, -1, window_size * num_points) + self.last_conv(
            x.view(batch_size, -1, window_size * num_points)
        )
...
class TemporalPointAE(nn.Module):
    def __init__(self, input_dim, latent_dim, window_size):
        ...
        self.block_3 = STPointNetBlock(
            256, [256, 256, 256], transposed=True, global_feat=True
        )

        self.enc_rnn = nn.GRU(
            256, 256, num_layers=1, batch_first=True, bidirectional=True
        )
    ...
    def decode(self, z, dec_cond):
        ...
        corr_mask = self.dec_mask(
            x.view(batch_size, -1, window_size * num_points)
        ).view(batch_size, 1, window_size, num_points)
        ...
        x = (
            torch.cat([corr_mask, corr_pts, corr_dist], dim=1)
            .permute(0, 2, 3, 1)
            .contiguous()
        )
        return x
