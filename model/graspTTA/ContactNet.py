...
class pointnet_reg(nn.Module):
    def __init__(self, num_class=1, with_rgb=True, n_pts=3000):
        ...
        if with_rgb:
            ...
            channel = 3
        self.k = num_class
        self.feat_o = PointNetEncoder(
            global_feat=False, feature_transform=False, channel=channel
        )  # feature trans True
        self.feat_h = PointNetEncoder(
            global_feat=False, feature_transform=False, channel=channel
        )  # feature trans True
        ...
        self.convfuse = torch.nn.Conv1d(3778, n_pts, 1)
        self.bnfuse = nn.BatchNorm1d(n_pts)
        self.n_pts = n_pts
    ...
    def forward(self, x, hand):
        ...
        # for obj
        x, trans, trans_feat = self.feat_o(
            x
        )  # x: [B, 1088, N] global+point feature of object
        # for hand
        hand, trans2, trans_feat2 = self.feat_h(
            hand
        )  # hand: [B, 1088, 778] global+point feature of hand
        # fuse feature of object and hand
        x = (
            torch.cat((x, hand), dim=2).permute(0, 2, 1).contiguous()
        )  # [B, N+778, 1088]
